...
public class UserServiceImpl implements UserService {
	...
	public String registerUser(UserDto userDto, BindingResult result, HttpSession session) {
		...
		if (result.hasErrors())
			...
			String role = userDto.getEmail().equalsIgnoreCase("bailpattar.raju1@gmail.com") ? "ROLE_ADMIN" : "ROLE_USER";
			User user = new User(null, userDto.getName(), userDto.getEmail(), encoder.encode(userDto.getPassword()),
					userDto.getMobile(), userDto.getGender(), otp, false, role, null);
			userDao.save(user);
	...
	public String checkout(HttpSession session, Principal principal, ModelMap map) {
		...
		if (items.isEmpty()) {
			...
			try {
				...
				String orderIdStr;
				if ("your-razorpay-key-id".equals(razorpayKeyId) || razorpayKeyId == null || razorpayKeyId.trim().isEmpty()) {
					// Mock Razorpay Order ID for local testing
					orderIdStr = "order_mock_" + System.currentTimeMillis();
				} else {
				...
				map.put("razorpayOrderId", orderIdStr);
	...
	public String paymentSuccess(String paymentId, String orderId, String signature, String address,
		...
		try {
			...
			if ("your-razorpay-key-secret".equals(razorpayKeySecret) || razorpayKeySecret == null || razorpayKeySecret.trim().isEmpty() || "mock_signature".equals(signature)) {
				isValidSignature = true;
			} else {
				isValidSignature = Utils.verifySignature(data, signature, razorpayKeySecret);
			}
			...
			order.setTrackingNumber("TRK" + System.currentTimeMillis() + new Random().nextInt(1000));
			...
			notificationService.createAndSend(
				user,
				"Order Confirmed! 🎉",
				"Your order #" + order.getId() + " has been placed successfully. Total: ₹" + Math.round(totalAmount * 100.0) / 100.0,
				"PAYMENT",
				order.getId()
			);
